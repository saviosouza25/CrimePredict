...
def run_multi_pair_analysis(interval, horizon, lookback_period, mc_samples, epochs):
    ...
    status_text.text(f"✅ Análise concluída! {completed} pares analisados com sucesso.")
    ...
    st.success(f"🎯 Análise multi-pares concluída com {len(results)} oportunidades identificadas!")
    st.rerun()

def analyze_single_pair_for_multi(pair, interval, horizon, lookback_period, mc_samples, epochs):
    ...
    try:
        ...
        predictions = calculate_unified_predictions(
            liquidity_analysis, technical_analysis, sentiment_analysis, ai_analysis
        )
        
        # 7. Determinar perfil operacional
        operational_profile = determine_operational_profile(
            predictions, liquidity_analysis, technical_analysis
        )
        
        # 8. Calcular validade temporal da análise
        analysis_validity = calculate_analysis_validity(interval, operational_profile, predictions)
        ...
        return {
            ...
            'technical_strength': technical_analysis.get('overall_strength', 0),
...
def calculate_unified_predictions(liquidity_analysis, technical_analysis, sentiment_analysis, ai_analysis):
    ...
    # Extrair scores normalizados de cada componente (0-100)
    liquidity_score = liquidity_analysis.get('overall_score', 50)
    
    # Análise técnica: converter força para score 0-100
    technical_strength = technical_analysis.get('overall_strength', 0)
    technical_score = 50 + (technical_strength * 25)  # -1 to 1 -> 25 to 75
    
    # Sentimento: converter compound score para 0-100
    sentiment_compound = sentiment_analysis.get('compound_score', 0)
    sentiment_score = 50 + (sentiment_compound * 50)  # -1 to 1 -> 0 to 100
    
    # IA: usar confiança diretamente
    ai_confidence = ai_analysis.get('confidence', 50)
    ai_prediction = ai_analysis.get('prediction', 'LATERAL')
    
    # Determinar direção predominante de cada componente
    components = {
        'liquidez': 'ALTA' if liquidity_score > 60 else 'BAIXA' if liquidity_score < 40 else 'NEUTRA',
        'tecnica': 'ALTA' if technical_score > 60 else 'BAIXA' if technical_score < 40 else 'NEUTRA',
        'sentimento': 'POSITIVO' if sentiment_score > 60 else 'NEGATIVO' if sentiment_score < 40 else 'NEUTRO',
        'ia': ai_prediction
    }
    
    # Calcular votos por direção
    alta_votes = sum(1 for comp in components.values() if comp in ['ALTA', 'POSITIVO', 'COMPRA'])
    baixa_votes = sum(1 for comp in components.values() if comp in ['BAIXA', 'NEGATIVO', 'VENDA'])
    neutra_votes = 4 - alta_votes - baixa_votes
    
    # Calcular scores médios ponderados
    weights = {'liquidez': 0.25, 'tecnica': 0.30, 'sentimento': 0.20, 'ia': 0.25}
    
    weighted_score = (
        liquidity_score * weights['liquidez'] +
        technical_score * weights['tecnica'] +
        sentiment_score * weights['sentimento'] +
        ai_confidence * weights['ia']
    )
    
    # Converter para probabilidades percentuais
    if weighted_score > 60:
        # Cenário de alta
        high_prob = min(85, 50 + (weighted_score - 60) * 1.5)
        low_prob = max(5, 30 - (weighted_score - 60) * 0.8)
        neutral_prob = 100 - high_prob - low_prob
    elif weighted_score < 40:
        # Cenário de baixa
        low_prob = min(85, 50 + (40 - weighted_score) * 1.5)
        high_prob = max(5, 30 - (40 - weighted_score) * 0.8)
        neutral_prob = 100 - high_prob - low_prob
    else:
        # Cenário neutro/indeciso
        neutral_prob = min(70, 40 + abs(weighted_score - 50) * 0.6)
        high_prob = (100 - neutral_prob) / 2
        low_prob = (100 - neutral_prob) / 2
    
    # Ajuste final baseado em consenso
    consensus_factor = max(alta_votes, baixa_votes, neutra_votes) / 4
    
    return {
        'alta_probabilidade': round(high_prob, 1),
        'baixa_probabilidade': round(low_prob, 1),
        'indecisao_probabilidade': round(neutral_prob, 1),
        'direcao_predominante': 'ALTA' if high_prob > max(low_prob, neutral_prob) else 
                               'BAIXA' if low_prob > neutral_prob else 'INDECISA',
        'consenso_componentes': consensus_factor,
        'score_unificado': round(weighted_score, 1),
        'componentes_detalhes': components
    }

def determine_operational_profile(predictions, liquidity_analysis, technical_analysis):
    """Determina o perfil operacional baseado nas análises"""
    
    # Fatores para determinação do perfil
    liquidity_score = liquidity_analysis.get('overall_score', 50)
    volatility = technical_analysis.get('volatility', 0.5)
    trend_strength = abs(technical_analysis.get('overall_strength', 0))
    consensus = predictions.get('consenso_componentes', 0)
    
    # Critérios para cada perfil
    if liquidity_score < 30 or volatility > 0.8:
        # Alta volatilidade ou baixa liquidez = Scalping
        profile = 'scalping'
        timeframe_rec = '1min-5min'
        holding_time = '15min-2h'
        risk_level = 'Alto'
        
    elif trend_strength > 0.6 and consensus > 0.75:
        # Tendência forte e consenso = Position Trading
        profile = 'position'
        timeframe_rec = '4h-1D'
        holding_time = '1-4 semanas'
        risk_level = 'Baixo'
        
    elif trend_strength > 0.4 and liquidity_score > 60:
        # Tendência moderada e boa liquidez = Swing Trading
        profile = 'swing'
        timeframe_rec = '1h-4h'
        ...
        # Condições mistas = Intraday
        profile = 'intraday'
        timeframe_rec = '15min-1h'
        holding_time = '2-8 horas'
        risk_level = 'Moderado'
    
    return {
        'tipo': profile,
        'timeframe_recomendado': timeframe_rec,
        'tempo_operacao': holding_time,
...
def calculate_analysis_validity(interval, operational_profile, predictions):
    """Calcula por quanto tempo a análise será válida"""
    
    # Tempo base por perfil operacional (em horas)
    base_validity = {
        'scalping': 2,      # 2 horas
        'intraday': 8,      # 8 horas  
        'swing': 48,        # 2 dias
        'position': 168     # 1 semana
    }
    
    profile_type = operational_profile.get('tipo', 'swing')
    base_hours = base_validity.get(profile_type, 24)
    
    # Ajustes baseados no consenso
    consensus = predictions.get('consenso_componentes', 0)
    if consensus > 0.8:
        validity_multiplier = 1.5  # Consenso alto = análise mais durável
    elif consensus < 0.4:
        validity_multiplier = 0.6  # Consenso baixo = análise menos durável
    else:
        validity_multiplier = 1.0
    
    # Ajuste baseado na força da tendência
    score_unificado = predictions.get('score_unificado', 50)
    if abs(score_unificado - 50) > 20:
        trend_multiplier = 1.2  # Tendência forte = mais durável
    else:
        trend_multiplier = 0.8  # Tendência fraca = menos durável
    
    # Calcular validade final
    final_validity_hours = base_hours * validity_multiplier * trend_multiplier
    
    # Converter para formato legível
    if final_validity_hours < 1:
        validity_text = f"{int(final_validity_hours * 60)} minutos"
    elif final_validity_hours < 24:
        validity_text = f"{final_validity_hours:.1f} horas"
    else:
        days = final_validity_hours / 24
        validity_text = f"{days:.1f} dias"
    
    return {
        'horas': round(final_validity_hours, 1),
        'texto': validity_text,
        'expira_em': datetime.now() + timedelta(hours=final_validity_hours),
        'fatores_ajuste': {
            'consenso_mult': validity_multiplier,
            'tendencia_mult': trend_multiplier,
            'base_perfil': base_hours
        }
    }

def display_multi_pair_results():
    """Exibe resultados da análise multi-pares com perfis operacionais e previsões"""
    
    results_data = st.session_state.get('multi_pair_results', {})
    if not results_data:
        st.warning("Nenhum resultado de análise multi-pares encontrado. Execute uma análise primeiro.")
        return
    ...
    with col1:
        st.metric("Pares Analisados", len(results))
    with col2:
        alta_count = sum(1 for r in results if r['predictions']['direcao_predominante'] == 'ALTA')
        st.metric("Sinais de Alta", alta_count)
    with col3:
        baixa_count = sum(1 for r in results if r['predictions']['direcao_predominante'] == 'BAIXA')
        st.metric("Sinais de Baixa", baixa_count)
    with col4:
        indeciso_count = len(results) - alta_count - baixa_count
        st.metric("Indecisões", indeciso_count)
    ...
    with filter_col1:
        direction_filter = st.selectbox(
            ...
            ["Todas", "ALTA", "BAIXA", "INDECISA"]
        )
    with filter_col2:
        profile_filter = st.selectbox(
            "Perfil Operacional", 
            ["Todos", "scalping", "intraday", "swing", "position"]
        )
    with filter_col3:
        min_consensus = st.slider("Consenso Mínimo", 0.0, 1.0, 0.5, 0.1)
    
    # Filtrar resultados
    filtered_results = []
    for result in results:
        # Filtro de direção
        if direction_filter != "Todas" and result['predictions']['direcao_predominante'] != direction_filter:
            continue
        
        # Filtro de perfil
        if profile_filter != "Todos" and result['operational_profile']['tipo'] != profile_filter:
            continue
        
        # Filtro de consenso
        if result['predictions']['consenso_componentes'] < min_consensus:
            continue
        
        filtered_results.append(result)
    
    # Ordenar por score unificado
    filtered_results.sort(key=lambda x: x['predictions']['score_unificado'], reverse=True)
    
    if not filtered_results:
        st.warning("Nenhum resultado encontrado com os filtros aplicados.")
        return
    
    st.info(f"📊 Exibindo {len(filtered_results)} resultados filtrados")
    
    # Exibir resultados
    for i, result in enumerate(filtered_results[:15]):  # Top 15
        pair = result['pair']
        predictions = result['predictions']
        profile = result['operational_profile']
        validity = result['analysis_validity']
        
        # Determinar cor baseada na direção
        if predictions['direcao_predominante'] == 'ALTA':
            color = "#00C851"
            icon = "📈"
        elif predictions['direcao_predominante'] == 'BAIXA':
            color = "#F44336"
            icon = "📉"
        else:
            color = "#FF9800"
        ...
        with st.expander(f"{icon} **{pair}** - {predictions['direcao_predominante']} ({predictions['score_unificado']:.1f}/100)"):
            
            # Primeira linha: Previsões percentuais
            pred_col1, pred_col2, pred_col3 = st.columns(3)
            with pred_col1:
                st.metric("🔼 Probabilidade Alta", f"{predictions['alta_probabilidade']:.1f}%")
            with pred_col2:
                st.metric("🔽 Probabilidade Baixa", f"{predictions['baixa_probabilidade']:.1f}%")
            with pred_col3:
                st.metric("➡️ Probabilidade Indecisão", f"{predictions['indecisao_probabilidade']:.1f}%")
            
            # Segunda linha: Perfil operacional e validade
            prof_col1, prof_col2 = st.columns(2)
            with prof_col1:
                st.markdown(f"""
                **🎯 Perfil Operacional: {profile['tipo'].title()}**
                - Timeframe: {profile['timeframe_recomendado']}
                - Tempo de operação: {profile['tempo_operacao']}
                - Nível de risco: {profile['nivel_risco']}
                """)
            
            with prof_col2:
                st.markdown(f"""
                **⏰ Validade da Análise**
                - Válida por: {validity['texto']}
                - Expira em: {validity['expira_em'].strftime('%d/%m %H:%M')}
                - Consenso: {predictions['consenso_componentes']:.1%}
                """)
            
            # Terceira linha: Detalhes dos componentes
            st.markdown("**📊 Análise Detalhada dos Componentes:**")
            comp_col1, comp_col2, comp_col3, comp_col4 = st.columns(4)
            
            with comp_col1:
                liq_score = result['liquidity_score']
                liq_color = "#00C851" if liq_score > 60 else "#FF9800" if liq_score > 40 else "#F44336"
                st.markdown(f"💧 **Liquidez**: <span style='color:{liq_color}'>{liq_score:.0f}/100</span>", unsafe_allow_html=True)
            
            with comp_col2:
                tech_strength = result['technical_strength']
                tech_score = 50 + (tech_strength * 25)
                tech_color = "#00C851" if tech_score > 60 else "#FF9800" if tech_score > 40 else "#F44336"
                st.markdown(f"📈 **Técnica**: <span style='color:{tech_color}'>{tech_score:.0f}/100</span>", unsafe_allow_html=True)
            
            with comp_col3:
                sent_score = 50 + (result['sentiment_score'] * 50)
                sent_color = "#00C851" if sent_score > 60 else "#FF9800" if sent_score > 40 else "#F44336"
                st.markdown(f"💭 **Sentimento**: <span style='color:{sent_color}'>{sent_score:.0f}/100</span>", unsafe_allow_html=True)
            
            with comp_col4:
                ai_conf = result['ai_confidence']
                ai_color = "#00C851" if ai_conf > 70 else "#FF9800" if ai_conf > 50 else "#F44336"
                st.markdown(f"🤖 **IA/LSTM**: <span style='color:{ai_color}'>{ai_conf:.0f}/100</span>", unsafe_allow_html=True)
    ...
    with col1:
        if st.button("🔄 Nova Análise Multi-Pares"):
            if 'multi_pair_results' in st.session_state:
